# weather-data-api2-cli

boru-driven command-line client **and** interactive REPL for the WeatherDataApi2
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/weather-data-api2-cli)
make build

# 2. See usage (words, entities, env vars)
./weather-data-api2-cli --help

# 3. Provide credentials once, via the environment
export WEATHER_DATA_API2_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./weather-data-api2-cli list weather

# 5. Override the API base URL for a single call
WEATHER_DATA_API2_BASE=https://api.example.com ./weather-data-api2-cli list weather

# 6. No arguments -> interactive REPL
./weather-data-api2-cli
weather-data-api2> list weather
weather-data-api2> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/weather-data-api2-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/weather-data-api2-cli list weather
   ```
...
Run with no arguments to open a REPL (prompt `weather-data-api2>`). Each line is
evaluated as its own boru expression:

```text
$ ./weather-data-api2-cli
weather-data-api2> list weather
weather-data-api2> /help
weather-data-api2> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `weather`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/weather-data-api2-cli`. |
...
(`./weather-data-api2-cli <expr>`) and interactively (the REPL), and expressions compose the
