...
class TestWeatherEntity:
    ...
    def test_should_stream(self):
        ...
        base = WeatherDataApi2SDK.test(seed, None)
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set WEATHER_DATA_API2_TEST_WEATHER_ENTID JSON to run live")
        client = setup["client"]
...
def _weather_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("WEATHER_DATA_API2_TEST_WEATHER_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("WEATHER_DATA_API2_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            ...
            {
                "apikey": env.get("WEATHER_DATA_API2_APIKEY"),
            },
            extra or {},
        ])
        client = WeatherDataApi2SDK(helpers.to_map(merged_opts))

    _live = env.get("WEATHER_DATA_API2_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("WEATHER_DATA_API2_TEST_EXPLAIN") == "TRUE",
        "live": _live,
