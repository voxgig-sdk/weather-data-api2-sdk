# WeatherDataApi2 SDK test runner
...
class WeatherDataApi2TestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            WeatherDataApi2TestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = WeatherDataApi2TestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = WeatherDataApi2TestRunner.getenv("WEATHER_DATA_API2_TEST_LIVE")
        override = WeatherDataApi2TestRunner.getenv("WEATHER_DATA_API2_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = WeatherDataApi2TestRunner.getenv(key)
        ...
        explain = WeatherDataApi2TestRunner.getenv("WEATHER_DATA_API2_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if WeatherDataApi2TestRunner._test_control is not None:
            return WeatherDataApi2TestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                WeatherDataApi2TestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            WeatherDataApi2TestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = WeatherDataApi2TestRunner.load_test_control()
...
def load_env_local():
    WeatherDataApi2TestRunner.load_env_local()


def env_override(m):
    return WeatherDataApi2TestRunner.env_override(m)


def entity_data(v):
    return WeatherDataApi2TestRunner.entity_data(v)


def entity_list_to_data(lst):
    return WeatherDataApi2TestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return WeatherDataApi2TestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return WeatherDataApi2TestRunner.load_test_control()


def live_client_options():
    return WeatherDataApi2TestRunner.live_client_options()


def live_delay_ms():
    return WeatherDataApi2TestRunner.live_delay_ms()
