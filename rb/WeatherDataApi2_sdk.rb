...
require_relative 'WeatherDataApi2_types'


class WeatherDataApi2SDK
  ...
  def initialize(options = {})
    ...
    utility = WeatherDataApi2Utility.new
    @_utility = utility
    ...
    feature_opts = WeatherDataApi2Helpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = WeatherDataApi2Helpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, WeatherDataApi2Features.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    WeatherDataApi2Utility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = WeatherDataApi2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = WeatherDataApi2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = WeatherDataApi2Helpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => WeatherDataApi2Error.new(
        "#{op}_allow",
        "WeatherDataApi2SDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue WeatherDataApi2Error => err
    ...
    if fetched.is_a?(Hash)
      status = WeatherDataApi2Helpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = WeatherDataApi2Error.new(
        "graphql_error", "WeatherDataApi2SDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = WeatherDataApi2SDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
