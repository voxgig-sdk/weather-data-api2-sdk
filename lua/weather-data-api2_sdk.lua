...
require("weather-data-api2_types")
...
local WeatherDataApi2SDK = {}
WeatherDataApi2SDK.__index = WeatherDataApi2SDK
...
WeatherDataApi2SDK._make_feature = _make_feature


function WeatherDataApi2SDK.new(options)
  local self = setmetatable({}, WeatherDataApi2SDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function WeatherDataApi2SDK:options_map()
...
function WeatherDataApi2SDK:get_utility()
  return Utility.copy(self._utility)
end


function WeatherDataApi2SDK:get_root_ctx()
  return self._rootctx
end


function WeatherDataApi2SDK:prepare(fetchargs)
...
function WeatherDataApi2SDK:direct(fetchargs)
...
function WeatherDataApi2SDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function WeatherDataApi2SDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "WeatherDataApi2SDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function WeatherDataApi2SDK:_raw_request(fetchargs)
  local utility = self._utility
...
function WeatherDataApi2SDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "WeatherDataApi2SDK: graphql: " .. msg
    res.graphql = errors
  end
...
function WeatherDataApi2SDK:Weather(data)
...
function WeatherDataApi2SDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = WeatherDataApi2SDK.new(sdkopts)
  sdk.mode = "test"
