...
class WeatherDataApi2MakeError
{
    public static function call(?WeatherDataApi2Context $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new WeatherDataApi2Context([], null);
        }
        $op = $ctx->op ?? new WeatherDataApi2Operation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new WeatherDataApi2Result([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof WeatherDataApi2Error) ? $err->msg : (string)$err;
        ...
        $sdk_err = new WeatherDataApi2Error('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof WeatherDataApi2Error) {
