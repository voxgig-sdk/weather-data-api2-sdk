...
module WeatherDataApi2Utilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = WeatherDataApi2Response.new({})
    ...
    if fetch_err
      ...
      response = WeatherDataApi2Response.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = WeatherDataApi2Response.new(fetched)
    else
