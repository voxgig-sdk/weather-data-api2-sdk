...
class WeatherDataApi2SDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = WeatherDataApi2Helpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = WeatherDataApi2Helpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!WeatherDataApi2Features::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, WeatherDataApi2Features::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return WeatherDataApi2Utility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = WeatherDataApi2Helpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = WeatherDataApi2Helpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = WeatherDataApi2Helpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new WeatherDataApi2Error($op . "_allow",
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = WeatherDataApi2Helpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new WeatherDataApi2Error("graphql_error",
                "WeatherDataApi2SDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new WeatherDataApi2SDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
