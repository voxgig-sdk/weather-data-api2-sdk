...
// WeatherDataApi2 SDK feature test
//
...
class FtCtrl extends WeatherDataApi2Control
{
...
class FtClient
{
    ...
    private WeatherDataApi2Utility $utility;

    public function __construct(WeatherDataApi2Utility $utility, array $options)
    {
    ...
    public function get_utility(): WeatherDataApi2Utility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (WeatherDataApi2Context $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public WeatherDataApi2Context $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (WeatherDataApi2Context $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = WeatherDataApi2Config::make_feature($name);
    ...
    private static function buildUrl(WeatherDataApi2Spec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof WeatherDataApi2Error) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new WeatherDataApi2Response($fetched);
        }
    ...
    private function fail(WeatherDataApi2Context $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (WeatherDataApi2Context $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
