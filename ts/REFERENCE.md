# WeatherDataApi2 TypeScript SDK Reference

Complete API reference for the WeatherDataApi2 TypeScript SDK.
...
```ts
new WeatherDataApi2SDK(options?: object)
```
...
#### `WeatherDataApi2SDK.test(testopts?, sdkopts?)`
...
**Returns:** `WeatherDataApi2SDK` instance in test mode.
...
Alias for `WeatherDataApi2SDK.test()`.
...
```ts
const results = await client.Weather().list({ appid: "example" })
```
...
Return the parent `WeatherDataApi2SDK` instance.
