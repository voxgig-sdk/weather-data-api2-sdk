...
//     (\`new WeatherDataApi2SDK(...)\` / \`WeatherDataApi2SDK.test(...)\` become
//     \`WeatherDataApi2SDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'WeatherDataApi2SDK'
...
// rewrite real client construction (\`new WeatherDataApi2SDK(...)\` and any
// \`WeatherDataApi2SDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+WeatherDataApi2SDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/WeatherDataApi2SDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
