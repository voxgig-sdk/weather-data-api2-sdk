# weather-data-api2-mcp

[MCP](https://modelcontextprotocol.io) server exposing the WeatherDataApi2 SDK as
two agent tools — `weather-data-api2_list` and `weather-data-api2_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/weather-data-api2-mcp)
make build

# 2. Provide credentials via the environment
export WEATHER_DATA_API2_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user weather-data-api2 \
  -- /absolute/path/to/weather-data-api2-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./weather-data-api2-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// weather-data-api2_list: first page of records
{ "entity": "weather" }
{ "entity": "weather", "query": { } }

// weather-data-api2_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/weather-data-api2-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user weather-data-api2 \
     -- "$PWD"/dist/*/weather-data-api2-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `weather-data-api2_list` and `weather-data-api2_load` tools now appear
   in new sessions. Ask the agent to *"list weather using weather-data-api2"*
   and it calls `weather-data-api2_list` with `{"entity":"weather"}`.
...
```sh
export WEATHER_DATA_API2_APIKEY=sk_live_xxx            # API key
export WEATHER_DATA_API2_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `weather-data-api2_list` tool
...
### Call the `weather-data-api2_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `weather-data-api2_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `weather-data-api2_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `WEATHER_DATA_API2_APIKEY` | API key sent with every request. |
| `WEATHER_DATA_API2_BASE` | Optional override of the API base URL. |
...
```sh
./weather-data-api2-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"weather-data-api2_load","arguments":{"entity":"weather","query":{"id":1}}}}'
```
